...
#define LGWR(data,len,fmt,...)	\
	lgwr_prt(__lgwr__handle,data,len,LGWRLEVELNOLEVEL,__FILE__,__LINE__, \
			fmt,##__VA_ARGS__)
#define LGWREMERGENCY(data,len,fmt,...) \
	if ((__lgwr__handle != NULL) && \
		(LGWRLEVELEMERGENCY <= ((struct lgwr_hd *)__lgwr__handle)->level)) { \
		lgwr_prt(__lgwr__handle,data,len,LGWRLEVELEMERGENCY,__FILE__,__LINE__, \
				fmt,##__VA_ARGS__); \
	}
#define LGWRALERT(data,len,fmt,...) \
	if ((__lgwr__handle != NULL) && \
		(LGWRLEVELALERT <= ((struct lgwr_hd *)__lgwr__handle)->level)) { \
		lgwr_prt(__lgwr__handle,data,len,LGWRLEVELALERT,__FILE__,__LINE__, \
				fmt,##__VA_ARGS__); \
	}
#define LGWRCRITICAL(data,len,fmt,...) \
	if ((__lgwr__handle != NULL) && \
		(LGWRLEVELCRITICAL <= ((struct lgwr_hd *)__lgwr__handle)->level)) { \
		lgwr_prt(__lgwr__handle,data,len,LGWRLEVELCRITICAL,__FILE__,__LINE__, \
				fmt,##__VA_ARGS__); \
	}
#define LGWRERROR(data,len,fmt,...) \
	if ((__lgwr__handle != NULL) && \
		(LGWRLEVELERROR <= ((struct lgwr_hd *)__lgwr__handle)->level)) { \
		lgwr_prt(__lgwr__handle,data,len,LGWRLEVELERROR,__FILE__,__LINE__, \
				fmt,##__VA_ARGS__); \
	}
#define LGWRWARN(data,len,fmt,...) \
	if ((__lgwr__handle != NULL) && \
		(LGWRLEVELWARN <= ((struct lgwr_hd *)__lgwr__handle)->level)) { \
		lgwr_prt(__lgwr__handle,data,len,LGWRLEVELWARN,__FILE__,__LINE__, \
				fmt,##__VA_ARGS__); \
	}
#define LGWRNOTICE(data,len,fmt,...) \
	if ((__lgwr__handle != NULL) && \
		(LGWRLEVELNOTICE <= ((struct lgwr_hd *)__lgwr__handle)->level)) { \
		lgwr_prt(__lgwr__handle,data,len,LGWRLEVELNOTICE,__FILE__,__LINE__, \
				fmt,##__VA_ARGS__); \
	}
#if SYS_LOG_LEVEL > 0
#define LGWRINFO(data,len,fmt,...) \
	if ((__lgwr__handle != NULL) && \
		(LGWRLEVELINFO <= ((struct lgwr_hd *)__lgwr__handle)->level)) { \
		lgwr_prt(__lgwr__handle,data,len,LGWRLEVELINFO,__FILE__,__LINE__, \
				fmt,##__VA_ARGS__); \
	}
#else
#define LGWRINFO(data,len,fmt,...)     do { } while (0)
#endif
#if (SYS_LOG_LEVEL > 1) && (!defined(DAP_CURRENT_MODULE) || (DAP_DEBUG_MODE_MODULES & DAP_CURRENT_MODULE))
#define LGWRDEBUG(data,len,fmt,...) \
	if ((__lgwr__handle != NULL) && \
		(LGWRLEVELDEBUG <= ((struct lgwr_hd *)__lgwr__handle)->level)) { \
		lgwr_prt(__lgwr__handle,data,len,LGWRLEVELDEBUG,__FILE__,__LINE__, \
				fmt,##__VA_ARGS__); \
	}
#else
...
#define LGWR(data,len,msg)	\
	lgwr_prt(__lgwr__handle,data,len,LGWRLEVELNOLEVEL,__FILE__,__LINE__,msg)
...
extern DLL_APP void  lgwr_prt(void *hd, const void *data, int len, int loglevel,
