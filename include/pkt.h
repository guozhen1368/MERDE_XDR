/*
 * (C) Copyright 2006
 * Hu Chunlin, chunlin.hu@[gmail|yahoo].com
 *
...
{
	return (pkthdr_get_type(ph) == PKTTYPE_CMD);
}
static __inline int pkthdr_slow_type_is_cmd(pkt_hdr *ph)
{
	return (pkthdr_slow_get_type(ph) == PKTTYPE_CMD);
}
#define TYPE_IS_CMD(type) \
	(((type) >= PKTTYPE_CMD_MIN) && ((type) <= PKTTYPE_CMD_MAX))
...
{
	return ((unsigned char *)ph + sizeof(pkt_hdr));
}
static __inline pkt_hdr *pkthdr_next(pkt_hdr *ph)
{
	return (pkt_hdr *)((unsigned char *)ph + pkthdr_get_plen(ph));
}
static __inline pkt_hdr *pkthdr_slow_next(pkt_hdr *ph)
{
	return (pkt_hdr *)((unsigned char *)ph + pkthdr_slow_get_plen(ph));
}
