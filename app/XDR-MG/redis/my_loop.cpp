...
void EventLoop::loop()
{
	...
	while (!quit_)
	{
			...
			it != activeChannels_.end(); ++it)
		{
			...
			{
				if (l->readCallback)l->readCallback(this, l->fd, l->privdata, 0);
			}

			if (l->events & EPoller::MY_WRITABLE)
			{
				f (l->writeCallback)l->writeCallback(this, l->fd, l->privdata, 0);
			}
...
void EventLoop::createFileEvent(int fd, int events, EventCallBack *callback, void *privdata)
{
	...
	if ( it == channels_.end())
	{
		...
		if ( NULL == chan)
		{
			LOGERROR("EventLoop : createFileEvent new Channel is null");
			return;
		}
