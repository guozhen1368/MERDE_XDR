...
# calling example
def main():
    configdict = ConvertXmlToDict('DataParse.xml')
    #pprint(configdict)
    #sorted(configdict['policy']['msgtype0'].items(), key=lambda d: d[0])
    for key in sorted(configdict['policy']['msgtype0']):
        print 'key=%s value=%s'%(key, configdict['policy']['msgtype0'][key])
    
    print configdict['policy']['msgtype0'].keys()

    # you can access the data as a dictionary
    print configdict['policy']['msgtype0']['s2']['len']
    #configdict['person']['gender'] = 'red'

    # or you can access it like object attributes
    #print configdict.address_book.person.gender
